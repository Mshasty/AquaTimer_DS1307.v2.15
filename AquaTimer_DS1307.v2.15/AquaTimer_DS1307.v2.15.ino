/********************************************************
* 6-channel timer for Aquarium. Version 2.15 (01.10.17) *
* creator: Owintsowsky Maxim     18 978 / 18 932 (1429) *
...
uint16_t  adc_key_val[5] ={92, 240, 380, 600, 820 };  // 08, 98, 255, 409, 640
...
sig8bit get_key(uint16_t input) { // Convert ADC value to key number
  sig8bit k;
...
void Read_Key() {
  ...
  if (key != oldkey) {            // if keypress is detected
    sig8bit newkey = key;
    myDelay(50);    // wait for debounce time
...
void loop() { // Main sketch loop
  ...
  if (oldsec != t.second()) {
    ...
    if (NotFeeding(TimeNow)) {
      snprintf(LCDbuff1, 17, "%02d:%02d:%02d        ", t.hour(), t.minute(), t.second());
      LCDbprint(2);
